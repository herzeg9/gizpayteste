...
function PhoneMock() {
  const [screen, setScreen] = useState<"home" | "pix" | "done">("home");

  function pay() {
    setScreen("pix");
    window.setTimeout(() => setScreen("done"), 1600);
  }
        ...
        <div className="flex min-h-[420px] flex-col gap-3 p-4">
              ...
              <div className="rounded-xl border-2 border-primary/20 bg-secondary/50 p-3">
                ...
                <div className="mt-2 grid grid-cols-2 gap-2">
                  <button
                    type="button"
                    onClick={pay}
                    className="grid h-8 place-items-center rounded-lg bg-primary text-[11px] font-semibold text-primary-foreground"
                  >
                    Pagar com Pix
                  </button>
                  <button
                    type="button"
                    onClick={pay}
                    className="grid h-8 place-items-center rounded-lg border bg-background text-[11px] font-semibold"
                  >
                    Boleto
                  </button>
                </div>
          ...
          {screen === "pix" ? (
            <div className="flex flex-1 flex-col items-center justify-center gap-3 py-8 text-center">
              <div className="grid size-28 place-items-center rounded-xl border border-dashed bg-card font-mono text-[10px] leading-4 text-muted-foreground">
                QR · Pix
                <br />
                Giz Pay
              </div>
              <p className="text-sm font-medium">Código copiado</p>
          ...
          {screen === "done" ? (
            <div className="flex flex-1 flex-col items-center justify-center gap-3 py-8 text-center">
              ...
              <p className="font-display text-2xl font-semibold">Pago</p>
              <p className="text-[11px] text-muted-foreground">
